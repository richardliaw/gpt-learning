...
def prefill(model: Transformer, x: torch.Tensor, input_pos: torch.Tensor, lengths: torch.Tensor, **sampling_kwargs) -> torch.Tensor:
    # input_pos: [B, S]
    logits = model(x, input_pos)
    # logits: [B, S, V]
    indices = torch.arange(0, lengths.shape[0])
    return sample(logits[indices, lengths - 1], **sampling_kwargs)[0]
...
PROMPT_LIST = ["Hello, my name is", "Luke, I am your", "NVIDIA stock in 2025 will be"]
def main(
    prompts: List[str] = PROMPT_LIST,
    ...
    prompt_lengths = {i: p.size(0) for i, p in enumerate(encoded_prompts)}

    from torch.nn.utils.rnn import pad_sequence
    encoded_prompts = pad_sequence(encoded_prompts, batch_first=True)
    print("Prompts:", encoded_prompts.shape)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prompt', type=str, default=PROMPT_LIST, help='Input prompt.')
